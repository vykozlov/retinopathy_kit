...
def get_metadata():
    ...
    meta['Name'] = "Retinopathy_InceptionV3"
...
def predict_file(img_path, model):
    ...
    """
    return gennet.predict_file(img_path, 'InceptionV3', model)
...
def predict_kaggle(test_path, file_list):
    """
    Simple call to gennet.predict_kaggle() using InceptionV3
    """    
    return gennet.predict_kaggle(test_path, file_list, 'InceptionV3')
...
def train(nepochs, model):
    ...
    """ 
    if model == 'cnn':
        return gennet.train_cnn(nepochs, 'InceptionV3')
    elif model == 'logreg':
        return gennet.train_logreg('InceptionV3')
